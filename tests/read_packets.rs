...
#[tokio::test]
async fn it_read_connect_user_name() {
    const DATA: &[u8] = &[
        0x10, 0x28, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0x82, 0x00, 0x3c, 0x00, 0x17, 0x6d,
        0x6f, 0x73, 0x71, 0x2d, 0x6e, 0x79, 0x6f, 0x31, 0x38, 0x6c, 0x39, 0x4c, 0x59, 0x52, 0x76,
        0x56, 0x61, 0x65, 0x73, 0x45, 0x76, 0x6b, 0x00, 0x03, 0x61, 0x73, 0x64,
    ];
    ...
    match packet {
        ControlPacket::Connect(connect) => {
            assert_eq!(connect.user_name, Some("asd".to_string()));
        }
        _ => panic!("Invalid packet type"),
    };
}
...
#[tokio::test]
async fn it_read_connect_password() {
    const DATA: &[u8] = &[
        0x10, 0x37, 0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0xc2, 0x00, 0x3c, 0x00, 0x17, 0x6d,
        0x6f, 0x73, 0x71, 0x2d, 0x76, 0x6b, 0x44, 0x62, 0x4f, 0x6d, 0x71, 0x74, 0x37, 0x31, 0x35,
        0x6a, 0x43, 0x30, 0x56, 0x56, 0x30, 0x72, 0x00, 0x08, 0x75, 0x73, 0x65, 0x72, 0x6e, 0x61,
        0x6d, 0x65, 0x00, 0x08, 0x70, 0x61, 0x73, 0x73, 0x77, 0x6f, 0x72, 0x64,
    ];
    ...
    match packet {
        ControlPacket::Connect(connect) => {
            assert_eq!(connect.password, Some("password".to_string()));
        }
        _ => panic!("Invalid packet type"),
    };
}
